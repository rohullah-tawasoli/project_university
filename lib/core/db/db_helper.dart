...
class DBHelper {
  ...
  static Future<void> initDb() async {
    ...
    try {
      ...
      _db =
        ...
        return db.execute(
          "CREATE TABLE $_tablename(id INTEGER PRIMARY KEY AUTOINCREMENT, title STRING, date STRING, rounds INTEGER, activityMinutes INTEGER, activitySeconds INTEGER, restMinutes INTEGER, restSeconds INTEGER)",
        );
      });
    } catch (e) {
      print(e);
  ...
  static Future<int> update(Note note) async {
    ...
    return await _db.rawUpdate(
      "UPDATE notes SET title = ?, rounds = ?, activityMinutes = ?, activitySeconds = ?, restMinutes = ?, restSeconds = ? WHERE id = ? ",
      [
        note.title,
        note.id,
        note.rounds,
        note.activityMinutes,
        note.activitySeconds,
        note.restMinutes,
        note.restSeconds
      ],
    );
